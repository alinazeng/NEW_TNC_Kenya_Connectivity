...
for(i in 1:length(cor.rmna)){
  ...
  sd = sd(cor.rmna[[i]])
  identifier <- 
  cor.dat.add <- data.frame(mean = mean, max= max, min = min, ninety_pct = ninety_pct, sd = sd, identifier = identifier)
...
line.dat<-data.frame(mean = numeric(), max= numeric(), min = numeric(), ninety_pct = numeric(), sd = numeric(),identifier = character())
